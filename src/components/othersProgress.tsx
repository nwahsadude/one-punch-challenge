...
function OthersProgress() {
  ...
  return (
    <div>
      ...
      <div>
        <ul
          className="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400 pt-2">
          {workouts.map(workout => {
            return (
              <li key={workout} className="mr-2">
                <span onClick={() => handleTabClick(workout)}
                      className={` ${workout === activeTab ? 'active dark:bg-gray-800 dark:text-blue-500' : ''} inline-block px-3 py-2 text-blue-600 bg-gray-100 rounded-t-lg hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300`}>
                  {workout}
                </span>
              </li>
            );
